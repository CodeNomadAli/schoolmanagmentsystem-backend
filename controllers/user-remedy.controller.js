...
export const getBookmarkedRemedies = async (req, res) => {
  try {
    ...
    const nameQuery = req.query.name || ""; // ✅ parse name from query
    let searchFilter = {};

    if (nameQuery) {
      searchFilter.name = {
        $regex: new RegExp(`(^${nameQuery}|${nameQuery}$)`, "i"), // ✅ starts with OR ends with
      };
    }
    ...
    if (!isFullAccess) {
      ...
      allowedRemedyIds = [...new Set(allowedRemedyIds.map((id) => id.toString()))];
    }

    const remediesRaw = await Remedy.find({
      _id: { $in: bookmarkedIds },
      ...searchFilter,
    })
      .select("name description media slug")
      .skip(skip)
      .limit(limit)
      .sort({ createdAt: -1 })
      .populate([
    ...
    const remedies = remediesRaw.map((remedy) => {
      const hasAccess =
