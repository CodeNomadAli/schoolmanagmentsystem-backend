...
export const createStaffRole = async (req, res) => {
  ...
  try {
    ...
    const existingRole = await StaffRole.findOne({ name: name.trim() }).lean().exec();
    if (existingRole) {
      await session.abortTransaction();
      session.endSession();
      return res
    ...
    const populatedRole = await StaffRole.findById(createRole[0]._id)
...
export const updateStaffRole = async (req, res) => {
  ...
  try {
    ...
    if (!id) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json(apiResponse(400, null, "Role ID is required"));
    }

    if (!req.body || Object.keys(req.body).length === 0) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json(apiResponse(400, null, "Request body cannot be empty"));
    }
    ...
    if (existingRole) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json(apiResponse(400, null, "Role with this name already exists"));
    }
    ...
    if (!updatedRole) {
      await session.abortTransaction();
      session.endSession();
      return res.status(404).json(apiResponse(404, null, "Staff role not found"));
    }

    await session.commitTransaction();
    session.endSession();

    return res.status(200).json(apiResponse(200, updatedRole, "Staff role updated successfully"));
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
...
export const deleteStaffRole = async (req, res) => {
  ...
  try {
    ...
    const Staff = (await import("../../models/staff.model.js")).default;
    const staffCount = await Staff.countDocuments({ staffRoleId: id }).session(session);

    if (staffCount > 0) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json(apiResponse(400, null, "Cannot delete role associated with staff members"));
    }

    const deletedRole = await StaffRole.findByIdAndDelete(id).session(session).lean().exec();
    ...
    return res.status(200).json(apiResponse(200, null, "Staff role deleted successfully"));
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
